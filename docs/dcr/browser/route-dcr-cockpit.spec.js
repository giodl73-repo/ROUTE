...
test.describe("ROUTE DCR cockpit", () => {
  test("desktop exposes the simulated operating board and claim boundaries", async ({ page }) => {
    ...
    await expect(page.getByLabel("Assurance case")).toContainText("Claimviolated drive SLA; P2 mitigate risk");
    await expect(page.locator("#assurance-case-status")).toHaveText("held");
    await expect(page.locator("#assurance-case-proof")).toHaveText("0/3 evidence; DCR-0; degraded");
    await expect(page.locator("#assurance-case-caveat")).toHaveText("held by 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#assurance-case-decision")).toHaveText("clear 511 closure feed");
    await expect(page.locator("#assurance-case-value")).toHaveText("T1 west gateway -> T1 east gateway; weather-responsive split + EV staging");
    await expect(page.locator("#assurance-case-renewal")).toHaveText("continue proof run");
    await expect(page.locator("#assurance-case-boundary")).toHaveText("assurance case only; no guaranteed SLA, ROI, or field release");
    await expect(page.getByLabel("Sponsor proof packet")).toContainText("Offercontinue monitored proof run");
    await expect(page.locator("#sponsor-proof-status")).toHaveText("held");
    await expect(page.locator("#sponsor-proof-line")).toHaveText("proof held until source custody clears; 0/3 evidence; DCR-0; degraded");
    await expect(page.locator("#sponsor-proof-value")).toHaveText("T1 west gateway -> T1 east gateway; weather-responsive split + EV staging");
    await expect(page.locator("#sponsor-proof-held")).toHaveText("511 closure feed; Charging source owner; Operator message owner; ROI; guaranteed SLA; traffic control; EV availability");
    ...
    await expect(page.getByLabel("Counterfactual value")).toContainText("Without DCRunmanaged corridor reliability failure; risk 46; flow 91%");
    await expect(page.locator("#counterfactual-value-status")).toHaveText("watch");
    await expect(page.locator("#counterfactual-with")).toHaveText("monitored violated drive SLA; P2 mitigate risk; risk 46; flow 91%");
    await expect(page.locator("#counterfactual-delta")).toHaveText("+0 risk; +0% flow");
    await expect(page.locator("#counterfactual-avoided")).toHaveText("clear source custody before sponsor decision");
    await expect(page.locator("#counterfactual-reason")).toHaveText("turns failures into accountable source work");
    await expect(page.locator("#counterfactual-gap")).toHaveText("held by 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#counterfactual-boundary")).toHaveText("counterfactual value is simulated; no ROI, revenue, or guaranteed SLA claim");
    await expect(page.getByLabel("Renewal operating contract")).toContainText("Serviceactive monitoring, DCR run decisions, source-gap governance");
    await expect(page.locator("#operating-contract-status")).toHaveText("gated");
    await expect(page.locator("#operating-contract-cadence")).toHaveText("0m; clear 3 source holds");
    await expect(page.locator("#operating-contract-rights")).toHaveText("operator recommends; 511 closure feed clears source custody");
    await expect(page.locator("#operating-contract-resilience")).toHaveText("P2 weather-responsive split + EV staging; +0 risk; +0% flow");
    await expect(page.locator("#operating-contract-deliverable")).toHaveText("continue proof run; source custody");
    ...
    await expect(page.getByLabel("Decision engine recommendation")).toContainText("Recommendclear 511 closure feed");
    await expect(page.locator("#decision-engine-status")).toHaveText("hold");
    await expect(page.locator("#decision-engine-why")).toHaveText("P2 corridor reliability failure; risk 46; flow 91%; recovery 42m");
    await expect(page.locator("#decision-engine-evidence")).toHaveText("source custody: 511 closure feed; 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#decision-engine-move")).toHaveText("verify 511 closure feed");
    await expect(page.locator("#decision-engine-gate")).toHaveText("not before Operator message owner clears");
    await expect(page.locator("#decision-engine-confidence")).toHaveText("low confidence; 0/3 evidence; DCR-0; degraded");
    await expect(page.locator("#decision-engine-blocked")).toHaveText("0/3 ready; missing 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#decision-engine-boundary")).toHaveText("decision engine recommendation only; operator and owner approval remain required");
    await expect(page.getByLabel("Operator approval packet")).toContainText("Requestclear 511 closure feed");
    await expect(page.locator("#operator-approval-status")).toHaveText("blocked");
    await expect(page.locator("#operator-approval-move")).toHaveText("verify 511 closure feed");
    await expect(page.locator("#operator-approval-evidence")).toHaveText("source custody: 511 closure feed; 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#operator-approval-owner")).toHaveText("511 closure feed");
    await expect(page.locator("#operator-approval-gate")).toHaveText("not before Operator message owner clears");
    ...
    await expect(page.getByLabel("Replay proof")).toContainText(/ReplayWinter closure and EV stress; 00:(00|05)/);
    ...
    await expect(page.getByLabel("Executive readout")).toHaveValue(/assurance_case_status,"held"/);
    ...
    await expect(page.getByLabel("Executive readout")).toHaveValue(/counterfactual_value_status,"watch"/);
    ...
    await expect(page.getByLabel("Executive readout")).toHaveValue(/decision_engine_status,"hold"/);
    ...
    await expect(page.locator("#remediation-status")).toHaveText("held");
    await expect(page.locator("#remediation-case")).toHaveText("DCR-FIX-WINTER-CLOSURE-P2");
    await expect(page.locator("#remediation-trigger")).toHaveText("promise risk 46>=45");
    await expect(page.locator("#remediation-action")).toHaveText("P2 weather-responsive split + EV staging");
    await expect(page.locator("#remediation-proof")).toHaveText("DCR-LEDGER-WINTER-CLOSURE-SAC-CENTRAL-VALLEY; 0/3; 3 source holds");
    await expect(page.locator("#remediation-gate")).toHaveText("source custody");
    await expect(page.locator("#remediation-boundary")).toHaveText("remediation docket only; owner issues and funds field fixes");
    await expect(page.getByLabel("Executive readout")).toHaveValue(/remediation_status,"held"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/remediation_action,"P2 weather-responsive split \+ EV staging"/);
    await expect(page.locator("#capital-status")).toHaveText("blocked");
    await expect(page.locator("#capital-lane")).toHaveText("source-clearance queue");
    await expect(page.locator("#capital-package")).toHaveText("DCR-MAINT-WINTER-CLOSURE-P2; DCR-FIX-WINTER-CLOSURE-P2");
    await expect(page.locator("#capital-priority")).toHaveText("P2; mountain corridor + charging sources");
    await expect(page.locator("#capital-value")).toHaveText("DCR-LEDGER-WINTER-CLOSURE-SAC-CENTRAL-VALLEY; 0/3; 3 source holds");
    await expect(page.locator("#capital-dependency")).toHaveText("source custody: verify 511 closure feed");
    await expect(page.locator("#capital-exit")).toHaveText("clear 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#capital-boundary")).toHaveText("capital queue only; no funding approval or procurement claim");
    await expect(page.getByLabel("Executive readout")).toHaveValue(/capital_status,"blocked"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/capital_lane,"source-clearance queue"/);
    await expect(page.locator("#memo-status")).toHaveText("blocked");
    await expect(page.locator("#memo-ask")).toHaveText("clear source custody before decision");
    await expect(page.locator("#memo-recommendation")).toHaveText("P2 weather-responsive split + EV staging");
    await expect(page.locator("#memo-proof")).toHaveText("DCR-LEDGER-WINTER-CLOSURE-SAC-CENTRAL-VALLEY; 0/3; 3 source holds");
    await expect(page.locator("#memo-blocked")).toHaveText("source custody: verify 511 closure feed");
    await expect(page.locator("#memo-authority")).toHaveText("DOT operations");
    await expect(page.locator("#memo-next")).toHaveText("clear 511 closure feed; Charging source owner; Operator message owner");
    await expect(page.locator("#memo-boundary")).toHaveText("decision memo only; owner decision and funding remain external");
    await expect(page.getByLabel("Executive readout")).toHaveValue(/memo_status,"blocked"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/memo_boundary,"decision memo only; owner decision and funding remain external"/);
  ...
  test("scheduled timeline events evolve risk and flow deterministically", async ({ page }) => {
    ...
    await expect(page.locator("#assurance-case-status")).toHaveText("held");
    await expect(page.locator("#assurance-case-claim")).toHaveText("maintained drive SLA; P3 maintain watch");
    await expect(page.locator("#assurance-case-proof")).toHaveText("1/3 evidence; DCR-4; degraded");
    await expect(page.locator("#assurance-case-caveat")).toHaveText("held by Charging source owner; Operator message owner");
    await expect(page.locator("#assurance-case-decision")).toHaveText("clear Charging source owner");
    await expect(page.locator("#assurance-case-renewal")).toHaveText("renew monitored DCR cockpit");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Assurance case: held; claim maintained drive SLA; P3 maintain watch; proof 1\/3 evidence; DCR-4; degraded; caveat held by Charging source owner; Operator message owner; decision clear Charging source owner; value T1 west gateway -> T1 east gateway; weather-responsive split \+ EV staging; renewal renew monitored DCR cockpit/);
    await expect(page.locator("#sponsor-proof-status")).toHaveText("held");
    await expect(page.locator("#sponsor-proof-offer")).toHaveText("continue monitored proof run");
    await expect(page.locator("#sponsor-proof-line")).toHaveText("DCR-LEDGER-WINTER-CLOSURE-CENTRAL-VALLEY-LA; DCR-4; -4 risk; +5% flow; 1 resolved; 1/3 evidence; DCR-4; degraded");
    await expect(page.locator("#sponsor-proof-value")).toHaveText("T1 west gateway -> T1 east gateway; weather-responsive split + EV staging");
    ...
    await expect(page.locator("#counterfactual-value-status")).toHaveText("proven");
    await expect(page.locator("#counterfactual-without")).toHaveText("unmanaged corridor reliability failure; risk 46; flow 91%");
    await expect(page.locator("#counterfactual-with")).toHaveText("monitored maintained drive SLA; P3 maintain watch; risk 42; flow 96%");
    await expect(page.locator("#counterfactual-delta")).toHaveText("-4 risk; +5% flow");
    await expect(page.locator("#counterfactual-avoided")).toHaveText("1 resolved; clear source custody before sponsor decision");
    await expect(page.locator("#counterfactual-reason")).toHaveText("keeps SLA proof and source gaps visible");
    await expect(page.locator("#counterfactual-gap")).toHaveText("held by Charging source owner; Operator message owner");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Counterfactual value: proven; without unmanaged corridor reliability failure; risk 46; flow 91%; with monitored maintained drive SLA; P3 maintain watch; risk 42; flow 96%; delta -4 risk; \+5% flow; avoided 1 resolved; clear source custody before sponsor decision; reason keeps SLA proof and source gaps visible; gap held by Charging source owner; Operator message owner/);
    await expect(page.locator("#operating-contract-status")).toHaveText("proof-run");
    await expect(page.locator("#operating-contract-cadence")).toHaveText("0m; clear 2 source holds");
    await expect(page.locator("#operating-contract-rights")).toHaveText("operator recommends; Charging source owner clears source custody");
    await expect(page.locator("#operating-contract-resilience")).toHaveText("P3 weather-responsive split + EV staging; -4 risk; +5% flow");
    await expect(page.locator("#operating-contract-deliverable")).toHaveText("renew monitored DCR cockpit; mountain corridor + charging sources");
    ...
    await expect(page.locator("#decision-engine-status")).toHaveText("prove");
    await expect(page.locator("#decision-engine-recommend")).toHaveText("clear Charging source owner");
    await expect(page.locator("#decision-engine-why")).toHaveText("P3 corridor reliability failure; risk 42; flow 96%; recovery 43m");
    await expect(page.locator("#decision-engine-evidence")).toHaveText("source custody: Charging source owner; Charging source owner; Operator message owner");
    await expect(page.locator("#decision-engine-move")).toHaveText("verify Charging source owner");
    await expect(page.locator("#decision-engine-gate")).toHaveText("not before Operator message owner clears");
    await expect(page.locator("#decision-engine-confidence")).toHaveText("low confidence; 1/3 evidence; DCR-4; degraded");
    await expect(page.locator("#decision-engine-blocked")).toHaveText("1/3 ready; missing Charging source owner; Operator message owner");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Decision engine recommendation: prove; recommend clear Charging source owner; why P3 corridor reliability failure; risk 42; flow 96%; recovery 43m; evidence source custody: Charging source owner; Charging source owner; Operator message owner; move verify Charging source owner; gate not before Operator message owner clears; confidence low confidence; 1\/3 evidence; DCR-4; degraded; blocked 1\/3 ready; missing Charging source owner; Operator message owner/);
    await expect(page.locator("#operator-approval-status")).toHaveText("source-review");
    await expect(page.locator("#operator-approval-request")).toHaveText("clear Charging source owner");
    await expect(page.locator("#operator-approval-move")).toHaveText("verify Charging source owner");
    await expect(page.locator("#operator-approval-evidence")).toHaveText("source custody: Charging source owner; Charging source owner; Operator message owner");
    await expect(page.locator("#operator-approval-owner")).toHaveText("Charging source owner");
    await expect(page.locator("#operator-approval-gate")).toHaveText("not before Operator message owner clears");
    ...
    await expect(page.locator("#remediation-status")).toHaveText("ready");
    await expect(page.locator("#remediation-case")).toHaveText("DCR-FIX-WINTER-CLOSURE-P3");
    await expect(page.locator("#remediation-trigger")).toHaveText("risk 42; flow 96%; recovery 43m");
    await expect(page.locator("#remediation-action")).toHaveText("P3 weather-responsive split + EV staging");
    await expect(page.locator("#remediation-proof")).toHaveText("mitigated; 1 resolved; 0 open; -4 risk; +5% flow");
    await expect(page.locator("#remediation-gate")).toHaveText("decide renewal candidate");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Remediation docket: ready; case DCR-FIX-WINTER-CLOSURE-P3; trigger risk 42; flow 96%; recovery 43m; action P3 weather-responsive split \+ EV staging; proof mitigated; 1 resolved; 0 open; -4 risk; \+5% flow; gate decide renewal candidate/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/remediation_status,"ready"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/remediation_replay_proof,"mitigated; 1 resolved; 0 open; -4 risk; \+5% flow"/);
    await expect(page.locator("#capital-status")).toHaveText("program-ready");
    await expect(page.locator("#capital-lane")).toHaveText("renewal/capital review");
    await expect(page.locator("#capital-package")).toHaveText("DCR-MAINT-WINTER-CLOSURE-P3; DCR-FIX-WINTER-CLOSURE-P3");
    await expect(page.locator("#capital-priority")).toHaveText("P3; mountain corridor + charging sources");
    await expect(page.locator("#capital-value")).toHaveText("DCR-4; -4 risk; +5% flow; 1 resolved");
    await expect(page.locator("#capital-dependency")).toHaveText("Charging source owner; Operator message owner");
    await expect(page.locator("#capital-exit")).toHaveText("owner decides renewal candidate");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Capital program queue: program-ready; lane renewal\/capital review; package DCR-MAINT-WINTER-CLOSURE-P3; DCR-FIX-WINTER-CLOSURE-P3; value DCR-4; -4 risk; \+5% flow; 1 resolved; dependency Charging source owner; Operator message owner; exit owner decides renewal candidate/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/capital_status,"program-ready"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/capital_value_basis,"DCR-4; -4 risk; \+5% flow; 1 resolved"/);
    await expect(page.locator("#memo-status")).toHaveText("decision-ready");
    await expect(page.locator("#memo-ask")).toHaveText("decide renewal candidate");
    await expect(page.locator("#memo-recommendation")).toHaveText("P3 weather-responsive split + EV staging");
    await expect(page.locator("#memo-proof")).toHaveText("DCR-4; -4 risk; +5% flow; 1 resolved");
    await expect(page.locator("#memo-blocked")).toHaveText("source gaps disclosed; not blocking memo");
    await expect(page.locator("#memo-authority")).toHaveText("DOT operations");
    await expect(page.locator("#memo-next")).toHaveText("review P3 fix package");
    await expect(page.getByLabel("Shift handoff")).toHaveValue(/Decision memo: decision-ready; ask decide renewal candidate; recommendation P3 weather-responsive split \+ EV staging; proof DCR-4; -4 risk; \+5% flow; 1 resolved; blocked source gaps disclosed; not blocking memo; authority DOT operations; next review P3 fix package/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/memo_status,"decision-ready"/);
    await expect(page.getByLabel("Executive readout")).toHaveValue(/memo_proof_basis,"DCR-4; -4 risk; \+5% flow; 1 resolved"/);
    ...
    await expect(page.getByLabel("Executive readout")).toHaveValue(/counterfactual_value_status,"proven"/);
    ...
    await expect(page.getByLabel("Executive readout")).toHaveValue(/decision_engine_status,"prove"/);
