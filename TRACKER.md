...
| T1/T1 failure-rate and reroute evidence ledger separates modeled assumptions from empirical closure data | 🔄 partial | `route t1-failures --gate-evidence` now enforces empirical/modeled/source_needed labels, confidence labels, artifacts, blockers, and next evidence steps; `route t1-diamond-validation --gate-catalog` tracks all 15 curated anchors and manual-validation blockers, with corrected Albert Lea/Benson/Toledo labels; most sites still need empirical sources |
