...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | `docs/game/des-moines-diamond-g0.md` now defines the G0 paper prototype with project cards, event cards, evidence cards, screen/copy/audio contracts, scoring, and session-log schema; next step is fixing the Des Moines diamond ledger mismatch or carrying it as a publication blocker |
