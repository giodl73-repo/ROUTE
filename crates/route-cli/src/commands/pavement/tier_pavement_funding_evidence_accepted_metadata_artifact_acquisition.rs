...
pub(crate) fn run(
    ...
    let review_rows = load_tier_pavement_funding_evidence_accepted_metadata_attachment_review(
        &accepted_metadata_attachment_review,
    )
    .with_context(|| format!("loading {}", accepted_metadata_attachment_review.display()))?;
    let rows =
        tier_pavement_funding_evidence_accepted_metadata_artifact_acquisition_rows(&review_rows);
    write_tier_pavement_funding_evidence_accepted_metadata_artifact_acquisition(&output, &rows)
