...
\paragraph{Voluntary vs.\ Mandatory Pricing and Uptake.}
The \$0.05/mile access fee can be implemented under two models with different
uptake dynamics. Under a \textbf{mandatory model} (all freight on managed lanes
pays the fee; access to managed lanes is freely granted to any enrolled carrier),
uptake is effectively 100\% for carriers that use the managed lane, but enrollment
may be incomplete in the first 3--5 years. Under a \textbf{voluntary model} (carriers
may choose GP lanes at zero cost; managed lane premium is a voluntary service
upgrade), uptake depends on value-of-time segmentation.

For a voluntary model, we estimate uptake using FMCSA carrier data segmented
by freight type:
\begin{itemize}
  \item Time-sensitive freight (SCTG 7, 8, 30 — perishables, chemicals, auto parts):
    $\sim$85\% uptake at \$0.05/mile (PTI improvement reduces premium to negligible)
  \item Standard truck freight (SCTG 1--6, 10--25 — bulk, grain, lumber):
    $\sim$40\% uptake (cost-sensitive; accepts some congestion)
  \item Intermodal/drayage (SCTG 40 — containerized): $\sim$60\% uptake
    (schedule-sensitive but rail alternative exists above threshold)
\end{itemize}

Under the voluntary model, blended uptake across freight types is approximately
62\%. This reduces toll revenue from the mandatory-assumption \$2.3B/yr to
\$1.4B/yr. However, voluntary pricing is politically more feasible and avoids
the statutory authority required for mandatory access fees on public highways.
The revised revenue trajectory under voluntary pricing recovers the capital
investment in approximately 22 years (vs.\ 14 years under mandatory pricing).
