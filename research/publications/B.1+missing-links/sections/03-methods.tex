...
The interstate graph does not include every on-ramp --- only major interchange nodes where two or more routes meet. This understates coverage for corridors with frequent ramp spacing. We expect this effect to be small relative to the gaps we identify.
...
We score each proposed corridor against the ROUTE 12-dimensional rubric as a proposed corridor (scores marked estimated). The coverage efficiency ranking is the primary ordering; the corridor score provides secondary context for strategic value beyond coverage.
