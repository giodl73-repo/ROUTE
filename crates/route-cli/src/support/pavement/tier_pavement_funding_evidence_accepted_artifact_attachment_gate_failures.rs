//! Helper `tier_pavement_funding_evidence_accepted_artifact_attachment_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_artifact_attachment_gate_failures(
    ...
    let expected = metadata_rows
        ...
        .map(|row| row.metadata_capture_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures.push(
            "funding evidence accepted artifact attachment has no held metadata-capture rows"
                .to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence accepted artifact attachment has {} rows but expected {} metadata-capture rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.accepted_artifact_attachment_id.trim().is_empty()
            || row.metadata_capture_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete accepted artifact attachment row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.metadata_capture_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.metadata_capture_id
            ));
        }
        if !expected.contains(row.metadata_capture_id.as_str()) {
            failures.push(format!(
                "{} is not a held source-needed metadata-capture row",
                row.metadata_capture_id
            ));
        }
        ...
        {
            failures.push(format!(
                "{} {} attaches unsupported funding evidence",
                row.state, row.route
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing accepted artifact attachment row for {expected_id}"
            ));
        }
    }
    failures
