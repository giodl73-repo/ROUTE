//! Helper `tier_pavement_funding_evidence_accepted_metadata_source_access_rows`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_metadata_source_access_rows(
    acquisition_rows: &[TierPavementFundingEvidenceAcceptedMetadataArtifactAcquisitionRow],
) -> Vec<TierPavementFundingEvidenceAcceptedMetadataSourceAccessRow> {
    acquisition_rows
        .iter()
        .filter(|row| {
        ...
        .map(
            |row| TierPavementFundingEvidenceAcceptedMetadataSourceAccessRow {
                accepted_metadata_source_access_id: format!(
                    "PAVEMENTFUNDINGACCEPTEDMETAACCESS-{}",
                    stable_id_fragment(&row.accepted_metadata_artifact_acquisition_id)
                ),
                accepted_metadata_artifact_acquisition_id: row
                ...
                next_action:
                    "collect manual or cached accepted funding artifact before intake and review"
                        .to_string(),
                next_artifact:
