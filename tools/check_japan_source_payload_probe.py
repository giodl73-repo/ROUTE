#!/usr/bin/env python3
"""Gate Japan source-payload probe output."""
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("Japan probe columns do not match contract")
    if len(probe_rows) != len(access_rows):
        failures.append("Japan probe row count does not match access manifest")
    ...
    if failures:
        print("Japan source-payload probe gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("Japan source-payload probe gate: PASS")
