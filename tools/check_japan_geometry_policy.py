#!/usr/bin/env python3
"""Gate Japan geometry policy before map or fixture use."""
...
POLICY = ROOT / "data" / "international-japan-geometry-policy-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("Japan geometry policy columns do not match contract")
    if len(rows) != 3:
        failures.append("Japan geometry policy must cover dry-run, map/media, and replacement surfaces")
    if not any("disaster-readiness" in row["blocked_uses"] or "disaster" in row["required_before_acceptance"] for row in rows):
        failures.append("Japan geometry policy must preserve disaster-readiness boundary")
    for row in rows:
    ...
    if failures:
        print("Japan geometry policy gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("Japan geometry policy gate: PASS")
    print("  checked no-geometry posture, disaster boundary, acceptance prerequisites, blocked uses, and claim holds")
    return 0
