...
| T1/T1 failure-rate and reroute evidence ledger separates modeled assumptions from empirical closure data | 🔄 partial | `route t1-failures --gate-evidence` now enforces empirical/modeled/source_needed labels, confidence labels, artifacts, blockers, and next evidence steps; curated analyzer anchors cover all 15 named T1/T1 pairs, with corrected Albert Lea/Benson/Toledo labels; most sites still need empirical sources |
