#!/usr/bin/env python3
"""Build India source-payload access manifest."""
...
OUTPUT = ROOT / "data" / "international-india-source-payload-access-001.csv"
...
def main() -> None:
    ...
    for source in read_csv(SOURCE_PACK):
        ...
        rows.append(
            {
                "payload_access_id": f"IND-PAYLOAD-{source_id.replace('IND-SRC-', '').replace('-', '')}",
                "source_id": source_id,
                "source_family": source["source_family"],
                "payload_url_or_status": source["source_path_or_status"],
                "owner_or_publisher": source["owner_or_publisher"],
                "cache_target": f"data/cache/india/{source_id.lower()}-payload.pending"
                ...
                "post_access_gate": "python tools/check_india_source_payload_access.py",
