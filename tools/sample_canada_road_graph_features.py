#!/usr/bin/env python3
"""Sample Canada road-graph feature attributes without accepting evidence."""
...
OUTPUT = ROOT / "data" / "international-canada-road-graph-feature-sample-001.csv"
USER_AGENT = "ROUTE-Canada-feature-sample/0.1 evidence-not-accepted"
...
def main() -> None:
    ...
    for index, feature in enumerate(fetch_features(), start=1):
        ...
        rows.append(
            {
                "sample_id": f"CAN-FEATURE-SAMPLE-{index:03d}",
                ...
                "sample_method": "esri-rest-query-no-geometry-limit-5",
                "geometry_status": "not-requested",
                "evidence_acceptance_status": "not-accepted",
                "blocked_claims": BLOCKED,
                "next_action": "map sampled attributes to parser candidates before any fixture replacement",
            }
        )
