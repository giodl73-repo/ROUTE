...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | `route game scenarios`, `inspect`, and deterministic `run-season` are implemented and covered by `docs/game/des-moines-diamond-cli-playtest-001.md` / `002.md`; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is active-project countdown plus `score` |
