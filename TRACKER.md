...
| Normalized event observation table computes annual T1/T1 failure rates and duration percentiles | 🔄 partial | `data/t1-failure-events.csv` and `route t1-failure-events --write-ledger` are ready; rows still need to be populated from DOT/FHWA source extracts |
