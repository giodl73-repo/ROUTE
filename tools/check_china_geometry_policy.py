#!/usr/bin/env python3
"""Gate China geometry policy before map or fixture use."""
...
POLICY = ROOT / "data" / "international-china-geometry-policy-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("China geometry policy columns do not match contract")
    if len(rows) != 3:
        failures.append("China geometry policy must cover dry-run, map/media, and replacement surfaces")
    if not any("policy-alignment" in row["blocked_uses"] or "policy-alignment" in row["required_before_acceptance"] for row in rows):
        failures.append("China geometry policy must preserve policy-alignment boundary")
    for row in rows:
    ...
    if failures:
        print("China geometry policy gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("China geometry policy gate: PASS")
    print("  checked no-geometry posture, policy boundary, acceptance prerequisites, blocked uses, and claim holds")
    return 0
