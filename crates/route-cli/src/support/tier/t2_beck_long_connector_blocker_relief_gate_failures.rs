//! Helper `t2_beck_long_connector_blocker_relief_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn t2_beck_long_connector_blocker_relief_gate_failures(
    ...
    if expected.is_empty() {
        failures.push("T2 long-connector blocker relief has no accepted policy rows".to_string());
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "T2 long-connector blocker relief has {} rows but expected {}",
            rows.len(),
            expected.len()
        ));
    }
    ...
    if actual_before != expected_before || actual_after != 0 {
        failures.push(format!(
            "T2 long-connector relief before/after = {actual_before}/{actual_after}, expected {expected_before}/0"
        ));
    }
    failures
}
