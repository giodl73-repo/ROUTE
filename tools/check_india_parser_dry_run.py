#!/usr/bin/env python3
"""Validate India parser dry-run fixture tables."""
...
TABLES = {
    "india_source_link_candidates": ROOT / "data" / "india_source_link_candidates.csv",
    "india_source_need_candidates": ROOT / "data" / "india_source_need_candidates.csv",
    "india_source_node_candidates": ROOT / "data" / "india_source_node_candidates.csv",
    "india_service_target_candidates": ROOT / "data" / "india_service_target_candidates.csv",
    "india_adapter_evidence_labels": ROOT / "data" / "india_adapter_evidence_labels.csv",
    "india_adapter_review_backlog": ROOT / "data" / "india_adapter_review_backlog.csv",
}
ROW_ID_FIELDS = {
    "india_source_link_candidates": "route_or_layer_id",
    "india_source_need_candidates": "need_id",
    "india_source_node_candidates": "node_id",
    "india_service_target_candidates": "target_gap_id",
}
ALLOWED_SOURCES = {
    "india_source_link_candidates": {"IND-SRC-002", "IND-SRC-005"},
    "india_source_need_candidates": {"IND-SRC-001", "IND-SRC-004"},
    "india_source_node_candidates": {"IND-SRC-003"},
}
ALLOWED_LABELS = {
    "india_source_link_candidates": {"source-candidate", "heuristic-held"},
...
def main() -> int:
    ...
    labels = {
        ...
        for row in table_rows["india_adapter_evidence_labels"]
    }
    ...
    for row in table_rows["india_source_link_candidates"]:
        if not row["geometry_ref"].startswith("not_accepted:"):
            failures.append("India link candidate accepted geometry")
    for row in table_rows["india_source_node_candidates"]:
        if "node identity and geometry not validated" not in row["access_note"]:
    ...
    for row in table_rows["india_adapter_review_backlog"]:
        if row["result"] != "pending":
            failures.append(f"role backlog result is not pending for {row['role_lane']}")
    if failures:
        print("India parser dry-run gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("India parser dry-run gate: PASS")
