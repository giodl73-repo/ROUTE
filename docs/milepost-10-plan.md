...
| 31 | Feed T1/T2 segment members into bundle registry | ✅ done | `route national-segment-registry --gate` now consumes `data/tier-segment-candidates.csv` and `data/tier-pavement-docket.csv`; pavement rows stay on members as debt/payment obligations rather than bundle identity blockers |
| 32 | Roll pavement debt up to bundle source actions | ✅ done | `route tier-pavement-source-gaps --gate` emits `data/tier-pavement-source-gaps.csv`, aggregating pavement source/repair debt into bundle-level source and payment actions |
...
| 36 | Gate release-manifest metadata | ✅ done | `route release-manifest --gate` validates release rows, artifact paths, allowed statuses, owner mileposts, and source-fetch policy ownership |
