#!/usr/bin/env python3
"""Probe EU Rhine-Alpine source-payload URLs without accepting evidence."""
...
OUTPUT = ROOT / "data" / "international-eu-rhine-alpine-source-payload-probe-001.csv"
USER_AGENT = "ROUTE-EU-Rhine-Alpine-payload-probe/0.1 evidence-not-accepted"
TIMEOUT_SECONDS = 20
SAMPLE_BYTES = 65536
...
def main() -> int:
    ...
    for access in read_csv(ACCESS):
        ...
        rows.append(
            {
                "probe_id": f"EUR-PROBE-{source_id.replace('EUR-SRC-', '').replace('-', '')}",
                "source_id": source_id,
