...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | `route game scenarios`, `inspect`, deterministic `run-season`, and active-project countdown are implemented and covered by `docs/game/des-moines-diamond-cli-playtest-001.md` / `002.md` / `003.md`; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is `score` |
