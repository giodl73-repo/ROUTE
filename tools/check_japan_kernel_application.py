#!/usr/bin/env python3
"""Gate Japan proof-kernel application ledger."""
...
LEDGER = ROOT / "data" / "international-japan-kernel-application-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("Japan kernel application columns do not match contract")
    if len(rows) != 5:
        failures.append("Japan kernel application must have five rows")
    steps = {row["kernel_step"] for row in rows}
    if steps != REQUIRED_STEPS:
        failures.append(f"Japan kernel steps mismatch: {sorted(steps)}")
    for row in rows:
        ...
        if step != "source_custody" and row["japan_status"] == "source_pack_preflight_declared":
    ...
    if failures:
        print("Japan kernel application gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("Japan kernel application gate: PASS")
