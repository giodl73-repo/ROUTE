//! Helper `tier_pavement_funding_evidence_accepted_artifact_acquisition_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_artifact_acquisition_gate_failures(
    ...
    let expected = review_rows
        ...
        .map(|row| row.accepted_attachment_review_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures.push(
            "funding evidence accepted artifact acquisition has no held review rows".to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence accepted artifact acquisition has {} rows but expected {} review rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.accepted_artifact_acquisition_id.trim().is_empty()
            || row.accepted_attachment_review_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete accepted artifact acquisition row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.accepted_attachment_review_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.accepted_attachment_review_id
            ));
        }
        if !expected.contains(row.accepted_attachment_review_id.as_str()) {
            failures.push(format!(
                "{} is not a held accepted attachment review row",
                row.accepted_attachment_review_id
            ));
        }
        ...
        {
            failures.push(format!(
                "{} {} has unsupported acquisition state",
                row.state, row.route
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing accepted artifact acquisition row for {expected_id}"
            ));
        }
    }
    failures
