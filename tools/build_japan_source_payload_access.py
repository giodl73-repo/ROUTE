#!/usr/bin/env python3
"""Build Japan source-payload access manifest."""
...
OUTPUT = ROOT / "data" / "international-japan-source-payload-access-001.csv"
...
def main() -> None:
    ...
    for source in read_csv(SOURCE_PACK):
        ...
        rows.append(
            {
                "payload_access_id": f"JPN-PAYLOAD-{source_id.replace('JPN-SRC-', '').replace('-', '')}",
                "source_id": source_id,
                "source_family": source["source_family"],
                "payload_url_or_status": source["source_path_or_status"],
                "owner_or_publisher": source["owner_or_publisher"],
                "cache_target": f"data/cache/japan/{source_id.lower()}-payload.pending"
                ...
                "post_access_gate": "python tools/check_japan_source_payload_access.py",
