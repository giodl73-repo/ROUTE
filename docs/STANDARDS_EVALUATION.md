...
The L1 inventory/source ledger for standards blocked on asset or operations data is `data/standards-l1-inventory.csv`.
...
Current status: Heuristic. `route diamond all`, `route t1-diamond-validation --gate-catalog`, `data/t1-diamond-validation.csv`, `data/t1-intersections.md`, and the B.4 paper establish the right framing. `route t1-diamond-validation --priority A --docket --with-access` is the working queue for top-site manual geometry, alternate-capacity, and observed-failure tasks. The curated analyzer catalog now covers all 15 named T1/T1 pairs, but manual validation and usable-throughput scenarios are still needed before this becomes a Blueprint-grade claim.
...
- MnDOT 511: the public app references a CARS backend, but direct CARS API/GraphQL requests returned 403 on May 9, 2026; treat it as access-blocked pending export access or an allowed public endpoint.
