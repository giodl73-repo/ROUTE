...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | `route game scenarios` and `route game inspect des-moines-diamond` are implemented and covered by `docs/game/des-moines-diamond-cli-playtest-001.md`; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is deterministic `run-season` |
