...
| Normalized event observation table computes annual T1/T1 failure rates and duration percentiles | 🔄 partial | `data/t1-failure-events.csv`, Iowa 511/MDOT/INDOT ingestion, TDOT SmartWay importer scaffolding, `route t1-accumulate-events`, `route t1-failure-events --gate-observations`, and `--write-ledger` are ready; TDOT live query still needs endpoint tuning, and annual history still needs accumulation |
