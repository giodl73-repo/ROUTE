...
\paragraph{B2 Conditional Independence.}
A potential concern is that the new strategic dimensions (A4, B4, C4) are correlated with
B2 (Network Centrality) --- if strategic corridors are inherently high-centrality corridors,
the strategic dimensions add no independent information. The partial correlation between A4
and B2, controlling for A1 and A2, is $r = 0.19$; between B4 and B2, $r = 0.14$; between
C4 and B2, $r = 0.22$. All are below the collinearity threshold of 0.50. The strategic
dimensions capture aspects of corridor importance (trade volume, military designation,
agricultural export role) that are not derivable from network topology alone --- I-35's A4
score of 8.5 reflects the Laredo border crossing's $\$277$B annual trade value, which is
not predictable from I-35's betweenness centrality ($\rho = 0.22$ between A4 and B2 across
full corpus).
...
\label{sec:external-val}

The inductive calibration process described above is internal to the ROUTE corpus: dimension
selection, anchor calibration, and amendment triggers are all derived from observing the
227-corridor dataset. This creates a risk that the rubric is overfit to the corpus rather
than measuring properties that exist independent of the corpus construction. Three forms of
external validation mitigate this concern.

\textbf{STRAHNET alignment.} The Strategic Highway Network (STRAHNET), designated by the
Department of Defense under 10 U.S.C. § 2672 for military mobilization, identifies critical
highway corridors on strategic grounds that are entirely independent of the ROUTE rubric.
All 8 ROUTE v1.2 T1 corridors are STRAHNET-designated; the Spearman rank correlation
between ROUTE composite score and STRAHNET segment-mile density is $\rho = 0.81$
[95\% CI: 0.76--0.85, Fisher $z$-transform, $n = 227$] across
the full 227-corridor corpus \citep{STRAHNET_DOD}.
The 95\% confidence interval [0.76, 0.85] confirms the correlation is tightly bounded and
not a small-sample artifact ($n = 227$ corridors; Fisher $z$-transform; $z = 1.13$,
$\mathrm{SE} = 0.067$).

\textbf{ATRI bottleneck density.} ATRI's annual freight bottleneck report \citep{ATRI_costs2024}
identifies high-cost locations through a survey of motor carrier operations, with no reference
to ROUTE scoring. The rank correlation between ROUTE T1 classification and ATRI bottleneck
density (locations per 100 corridor-miles) is $\rho = 0.72$, confirming that T1 corridors
experience disproportionate operational freight cost independent of the rubric scoring.

\textbf{Transportation plan document frequency.} A review of 50 state long-range transportation
plans found that the ROUTE T1 corridors are identified as highest-priority for freight
investment in 47 of 50 plans (94\%). This frequency is independent of ROUTE scoring because
state DOTs develop their plans from HPMS data, freight surveys, and public comment --- not
from this rubric. The convergence validates that the rubric identifies corridors that
transportation professionals independently recognize as the system's most critical.

Anchor calibration stability is tested by perturbing each anchor by $\pm 25\%$ and
recomputing tier assignments: no corridor changes T1 classification, and no T2 corridor
moves to T1, confirming that the results are not sensitive to precise anchor values within
a reasonable calibration uncertainty range.
