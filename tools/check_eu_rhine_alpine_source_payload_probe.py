#!/usr/bin/env python3
"""Gate EU Rhine-Alpine source-payload probe output."""
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("EU probe columns do not match contract")
    if len(probe_rows) != len(access_rows):
        failures.append("EU probe row count does not match access manifest")
    ...
    if failures:
        print("EU Rhine-Alpine source-payload probe gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1

    print("EU Rhine-Alpine source-payload probe gate: PASS")
