//! `TierPavementFundingEvidenceAcceptedSourceAccess` command handler extracted from main.
...
pub(crate) fn run(
    ...
    let acquisition_rows = load_tier_pavement_funding_evidence_accepted_artifact_acquisition(
    ...
    if gate {
        ...
        if !failures.is_empty() {
            ...
            anyhow::bail!("tier pavement funding evidence accepted source access gate failed");
        }
        println!();
