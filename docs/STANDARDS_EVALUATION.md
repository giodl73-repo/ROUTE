...
`route t1-fetch-iowa511` caches the current public Iowa DOT 511 ArcGIS event layer. `route t1-import-iowa511` normalizes nearby I-35/I-80 records into the T1/T1 event schema for source accumulation. `route t1-accumulate-events` merges generated source rows into the canonical event table and dedupes repeated site/event IDs. The Iowa feed is live, not a historical archive, so cache snapshots should be treated as observation samples until a polling or archive strategy creates a stable annual history.
