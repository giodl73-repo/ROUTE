...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | G1-A CLI loop now covers `scenarios`, `inspect`, `run-season`, and `score`, with playtests `001.md` through `004.md` and `data/game/des-moines-diamond-session-fixture.csv`; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is G1-B engine fact summaries |
