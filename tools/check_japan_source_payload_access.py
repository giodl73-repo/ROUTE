#!/usr/bin/env python3
"""Gate Japan source-payload access manifest."""
...
ACCESS = ROOT / "data" / "international-japan-source-payload-access-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("Japan payload access columns do not match contract")
    if len(access_rows) != len(source_rows):
        failures.append("Japan payload access row count does not match source pack")
    if access_ids != source_ids:
        failures.append("Japan payload access source IDs do not match source pack")
    url_rows = [row for row in access_rows if row["payload_url_or_status"].startswith("http")]
    if len(url_rows) < 6:
        failures.append("Japan payload access must retain six official URL probe candidates")
    for row in access_rows:
        ...
        for required in REQUIRED_BLOCKS:
            if required not in row["blocked_claims"]:
    ...
    if failures:
        print("Japan source-payload access gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("Japan source-payload access gate: PASS")
