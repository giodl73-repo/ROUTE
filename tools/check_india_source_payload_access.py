#!/usr/bin/env python3
"""Gate India source-payload access manifest."""
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("India payload access columns do not match contract")
    if len(access_rows) != len(source_rows):
        failures.append("India payload access row count does not match source pack")
    if access_ids != source_ids:
        failures.append("India payload access source IDs do not match source pack")
    url_rows = [row for row in access_rows if row["payload_url_or_status"].startswith("http")]
    if len(url_rows) < 4:
        failures.append("India payload access must retain four official URL probe candidates")
    ...
    if failures:
        print("India source-payload access gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("India source-payload access gate: PASS")
