//! Helper `tier_pavement_funding_evidence_source_access_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_source_access_gate_failures(
    ...
    let expected = acquisition_rows
        ...
        .map(|row| row.funding_evidence_acquisition_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures
            .push("funding evidence source access has no source-needed acquisitions".to_string());
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence source access has {} rows but expected {} acquisition rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.source_access_id.trim().is_empty()
            || row.funding_evidence_acquisition_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete funding evidence source-access row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.funding_evidence_acquisition_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.funding_evidence_acquisition_id
            ));
        }
        if !expected.contains(row.funding_evidence_acquisition_id.as_str()) {
            failures.push(format!(
                "{} is not a source-needed funding evidence acquisition row",
                row.funding_evidence_acquisition_id
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing funding evidence source-access row for {expected_id}"
            ));
        }
    }
    failures
