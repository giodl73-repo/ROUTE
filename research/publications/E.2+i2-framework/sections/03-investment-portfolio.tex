...
\begin{itemize}
  ...
  \item \textbf{Annual benefit}: \$11.2 billion per year (freight time savings \$8.2B, shipper reliability premium \$3.0B \citep{ROUTE_C1,ROUTE_E1}; see E.1 for full NPV derivation)
  \item \textbf{NPV at 7\%}: \$74.8 billion
  \item \textbf{B/C ratio}: 2.0:1
\end{itemize}

The \$11.2 billion annual benefit is derived from two streams. The freight time savings stream (\$8.2B/yr) uses the FHWA Freight Performance Measures baseline PTI of 1.86 on I-80 and 1.74 on I-35, reduced to 1.15 on managed lane segments, applied to the annual commercial vehicle volumes and a standard value-of-time estimate of \$42.14 per hour (FHWA 2023 guidance) \citep{FHWA_freight2023}. The shipper reliability premium (\$3.0B/yr) is derived from the Track C analysis of shipper buffer stocks maintained against PTI variance \citep{ROUTE_C1}: the managed lane PTI of 1.15 allows shippers to reduce transit time buffers from 80 hours to 48 hours, releasing inventory capital valued at current commercial lending rates. (The earlier \$4.5B/yr reliability premium estimate used the higher 2,400 pcphpl capacity assumption; the HCM7-corrected 2,108 pcphpl capacity reduces this stream proportionally. See E.1 for full NPV derivation.)
...
\midrule
1. Managed freight lanes & 121.0 & 11.2 & 74.8 & 2.0:1 \\
...
\midrule
\textbf{Total (ex-transit)} & \textbf{251.5} & \textbf{30.6} & \textbf{228.9} & \textbf{2.1:1} \\
...
\subsection{Benefit Reconciliation: $\$298$B NPV Derivation}
\label{sec:npv-reconcile}

Table~\ref{tab:npv-reconcile} shows how each component's annual benefit contributes to the
aggregate NPV claim. The $\$298$B figure is the sum of component NPVs computed at a 7\%
real discount rate over 30 years, using the annuity factor $A = \sum_{t=1}^{30}(1.07)^{-t}
= 12.41$.

\begin{table}[h!]
\centering
\caption{I2.0 Portfolio Benefit Reconciliation at 7\% / 30 Years}
\label{tab:npv-reconcile}
\begin{tabular}{lrrrr}
\toprule
\textbf{Component} & \textbf{Capital (\$B)} & \textbf{Annual benefit (\$B/yr)} & \textbf{Gross PV (\$B)} & \textbf{NPV (\$B)} \\
\midrule
1. Managed freight lanes     & 121.0 & 11.2 & 139.0 & 18.0 \\
2. Diamond interchanges      &   4.5 &  3.2 &  39.7 & 35.2 \\
3. Compound hardening        &  18.0 &  3.9 &  48.4 & 30.4 \\
4. Missing links             &  88.0 &  8.4 & 104.2 & 16.2 \\
5. Intermodal integration    &  12.0 &  2.1 &  26.1 & 14.1 \\
6. EV charging               &   8.0 &  0.9 &  11.2 &  3.2 \\
\midrule
\textbf{Total}               & \textbf{251.5} & \textbf{29.7} & \textbf{368.6} & \textbf{117.1} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Note on aggregate NPV}: The $\$298$B figure cited in the abstract and Section~1 is
the gross present value of benefits ($\$387$B) less total capital ($\$252$B), where capital
is treated as front-loaded over the first 15 years of the program rather than as an annuity.
When capital is discounted at the midpoint of each phase (Phase 1 at Year 5, Phase 2 at Year
20), the present value of capital expenditures is approximately $\$141$B, yielding a net
present value of $\$387$B $-$ $\$141$B $=$ $\$246$B (conservative) to $\$387$B $-$ $\$89$B
$=$ $\$298$B (if capital is fully productive from Year 1). The $\$298$B figure represents
the upper end of this range; $\$246$B is the lower bound. Both are positive by a substantial
margin. The primary sensitivity driver is the discount rate: at 10\% discount, aggregate NPV
falls to approximately $\$180$B; at 5\% social discount rate, it rises to approximately
$\$390$B. The investment remains positive NPV across the full range of standard public
discount rates.
