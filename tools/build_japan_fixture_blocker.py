#!/usr/bin/env python3
"""Build Japan fixture replacement blocker."""
...
ROW_VALIDATION = ROOT / "data" / "international-japan-source-content-row-validation-001.csv"
ROLE_REVIEW = ROOT / "data" / "international-japan-content-row-role-review-001.csv"
GEOMETRY_POLICY = ROOT / "data" / "international-japan-geometry-policy-001.csv"
OUTPUT = ROOT / "data" / "international-japan-fixture-blocker-001.csv"
...
def main() -> None:
    ...
    results = sorted({row["validation_result"] for row in validation_rows})
    ...
    rows = [
        {
            "blocker_id": "JPN-FIXTURE-BLOCKER-001",
            "replacement_target": "Japan dry-run link/node/need/target fixture tables",
            "current_rows": f"{len(validation_rows)} content-row matches with results {','.join(results)}",
            "role_review_status": role_status,
            "geometry_status": geometry_status,
            "replacement_decision": "blocked_source_rows_not_validated_gsi_link_source_needed_geometry_not_accepted",
            "allowed_use": "gap tracking and source acquisition planning only",
            "blocked_claims": BLOCKED_CLAIMS,
            "required_next_step": "resolve road-link source custody or write a no-geometry blocker closeout before any fixture replacement contract",
        }
    ]
