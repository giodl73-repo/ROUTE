...
Current status: Heuristic. `route diamond all`, `route t1-diamond-validation --gate-catalog`, `data/t1-diamond-validation.csv`, `data/t1-intersections.md`, and the B.4 paper establish the right framing. The curated analyzer catalog now covers all 15 named T1/T1 pairs, but manual validation and usable-throughput scenarios are still needed before this becomes a Blueprint-grade claim.
