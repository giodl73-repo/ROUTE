#!/usr/bin/env python3
"""Gate Canada geometry policy before map or fixture use."""
...
POLICY = ROOT / "data" / "international-canada-geometry-policy-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("geometry policy columns do not match required contract")
    if len(rows) != 3:
        failures.append("geometry policy must cover extraction, media/map, and replacement surfaces")
    for row in rows:
    ...
    if failures:
        print("Canada geometry policy gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("Canada geometry policy gate: PASS")
