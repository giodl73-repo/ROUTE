#!/usr/bin/env python3
"""Gate India proof-kernel application ledger."""
...
LEDGER = ROOT / "data" / "international-india-kernel-application-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("India kernel application columns do not match contract")
    if len(rows) != 5:
        failures.append("India kernel application must have five rows")
    steps = {row["kernel_step"] for row in rows}
    if steps != REQUIRED_STEPS:
        failures.append(f"India kernel steps mismatch: {sorted(steps)}")
    for row in rows:
        ...
        if step != "source_custody" and row["india_status"] == "source_pack_preflight_declared":
    ...
    if failures:
        print("India kernel application gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("India kernel application gate: PASS")
