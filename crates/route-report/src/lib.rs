...
fn format_corpus_entry(corridor: &Corridor, scores: &DimensionScores) -> String {
    ...
    md.push_str(&format!("  termini: [\"{}\", \"{}\"]\n",
        corridor.termini[0], corridor.termini[1]));
    md.push_str(&format!("  states: [{}]\n",
        corridor.states.iter().map(|s| format!("\"{s}\"")).collect::<Vec<_>>().join(", ")));
    ...
    md.push_str(&format!("| Total miles | {:.0} | NHS shapefile |\n", corridor.total_miles));
    md.push_str(&format!("| Mean AADT | {} | HPMS 2023 |\n",
        attrs.mean_aadt.map(|v| format!("{v:.0}")).unwrap_or("N/A".into())));
    md.push_str(&format!("| Mean truck % | {} | HPMS 2023 |\n",
        attrs.mean_pct_truck.map(|v| format!("{:.0}%", v * 100.0)).unwrap_or("N/A".into())));
    md.push_str(&format!("| Bridges | {} | NBI 2023 |\n", attrs.bridge_count));
    md.push_str(&format!("| Bridges poor | {} | NBI 2023 |\n",
        attrs.pct_bridges_poor.map(|v| format!("{:.0}%", v * 100.0)).unwrap_or("N/A".into())));
    md.push_str(&format!("| States | {} | NHS shapefile |\n", corridor.states.join(", ")));
    md.push('\n');
...
fn slug(designation: &str) -> String {
    designation.to_lowercase().replace('-', "-").replace(' ', "-")
}
