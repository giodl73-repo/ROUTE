...
| Normalized event observation table computes annual T1/T1 failure rates and duration percentiles | 🔄 partial | `data/t1-failure-events.csv`, `route t1-fetch-iowa511`, `route t1-import-iowa511`, `route t1-accumulate-events`, and `route t1-failure-events --write-ledger` are ready; Iowa rows can now be generated and deduped from live DOT snapshots, but annual history still needs accumulation |
