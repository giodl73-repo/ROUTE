//! Helper `tier_pavement_funding_evidence_accepted_metadata_artifact_attachment_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_metadata_artifact_attachment_gate_failures(
    rows: &[TierPavementFundingEvidenceAcceptedMetadataArtifactAttachmentRow],
    metadata_rows: &[TierPavementFundingEvidenceAcceptedMetadataCaptureRow],
) -> Vec<String> {
    let mut failures = Vec::new();
    let expected = metadata_rows
        .iter()
        .filter(|row| {
        ...
        .map(|row| row.accepted_metadata_capture_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures.push(
            "funding evidence accepted metadata artifact attachment has no held metadata-capture rows"
                .to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence accepted metadata artifact attachment has {} rows but expected {} metadata-capture rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row
            ...
            .trim()
            .is_empty()
            || row.accepted_metadata_capture_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete accepted metadata artifact-attachment row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.accepted_metadata_capture_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.accepted_metadata_capture_id
            ));
        }
        if !expected.contains(row.accepted_metadata_capture_id.as_str()) {
            failures.push(format!(
                "{} is not a held accepted metadata-capture row",
                row.accepted_metadata_capture_id
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing accepted metadata artifact-attachment row for {expected_id}"
            ));
        }
    }
    failures
