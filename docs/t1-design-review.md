...
- `I84` rises into the 11-route set as a conditional built-network exception. It now has Beck diagnostic coverage, but still needs a national relay justification before it should be treated like a promise-spine line.
...
The constraint order for that decision now lives in `docs/tier-optimizer-design.md`.
