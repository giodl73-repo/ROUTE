...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | G1-A CLI loop now covers `scenarios`, `inspect`, `run-season`, and `score`; G1-B score output derives Des Moines engine facts from route-sim and route-network; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is state fixture/browser planning |
