...
| Normalized event observation table computes annual T1/T1 failure rates and duration percentiles | 🔄 partial | `data/t1-failure-events.csv` now carries a small Iowa 511 normalized observation sample and passes `route t1-failure-events --gate-observations`; Iowa 511/INDOT snapshot polling is planned and gated by `route t1-snapshot-plan --gate-plan`, with `route t1-accumulate-events` and `--write-ledger` ready; annual history still needs accumulation |
