...
\medskip
\noindent\textbf{The full per-trip comparison including hub fees.}
On a per-trip basis, relay costs \$1,650 versus \$1,456 for solo driving once hub fees (\$100/swap $\times$ 6 swaps = \$600) are included: $C_{\text{relay,total}} = \$1{,}050 + \$600 = \$1{,}650$. Relay is therefore \textit{more expensive} per trip as a direct cost. The economic case for relay rests not on per-trip cost reduction but on asset utilization: relay trucks operate at 98\% utilization versus 48\% for solo drivers. The same revenue ton-miles require roughly half as many trucks under relay, reducing fleet capital cost by approximately \$450 million for a carrier running 500 transcontinental routes annually (500 routes $\times$ 260 operating days $\times$ \$8{,}700 capital savings per truck-year $\div$ 2,500 mi/route $\approx$ fleet-level savings at scale). The correct comparison is per-ton-mile delivered per dollar of fleet capital, not per-trip direct cost --- a comparison that favors relay by a factor of roughly 2.0$\times$ on any route exceeding 1,500 miles.
