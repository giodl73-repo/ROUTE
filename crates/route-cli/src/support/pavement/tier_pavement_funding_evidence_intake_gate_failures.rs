//! Helper `tier_pavement_funding_evidence_intake_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_intake_gate_failures(
    ...
    if expected.is_empty() {
        failures.push("funding evidence intake has no held source-access rows".to_string());
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence intake has {} rows but expected {} source-access rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.funding_evidence_intake_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete funding evidence intake row",
                row.state, row.route
            ));
        }
        ...
        if !expected.contains(row.source_access_id.as_str()) {
            failures.push(format!(
                "{} is not a held source-access row",
        ...
        {
            failures.push(format!(
                "{} {} has unsupported funding evidence intake status",
                row.state, row.route
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing funding evidence intake row for {expected_id}"
            ));
        }
    }
    failures
