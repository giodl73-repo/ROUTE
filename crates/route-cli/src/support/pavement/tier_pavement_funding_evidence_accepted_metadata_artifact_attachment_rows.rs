//! Helper `tier_pavement_funding_evidence_accepted_metadata_artifact_attachment_rows`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_metadata_artifact_attachment_rows(
    ...
    metadata_rows
        ...
        .map(
            |row| TierPavementFundingEvidenceAcceptedMetadataArtifactAttachmentRow {
                accepted_metadata_artifact_attachment_id: format!(
                    "PAVEMENTFUNDINGACCEPTEDMETAATTACH-{}",
                    stable_id_fragment(&row.accepted_metadata_capture_id)
                ),
                accepted_metadata_capture_id: row.accepted_metadata_capture_id.clone(),
