...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | G1-A CLI loop now covers `scenarios`, `inspect`, `run-season`, and `score`; G1-B has started with score-output engine facts for Des Moines; G0-C remains held for a human blind playtest or owner acceptance of simulated evidence; next step is deriving those facts directly from route-sim and route-network |
