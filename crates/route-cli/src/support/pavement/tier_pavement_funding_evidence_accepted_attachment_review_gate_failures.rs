//! Helper `tier_pavement_funding_evidence_accepted_attachment_review_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_attachment_review_gate_failures(
    ...
    let expected = attachment_rows
        ...
        .map(|row| row.accepted_artifact_attachment_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures.push(
            "funding evidence accepted attachment review has no held attachment rows".to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence accepted attachment review has {} rows but expected {} attachment rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.accepted_attachment_review_id.trim().is_empty()
            || row.accepted_artifact_attachment_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete accepted attachment review row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.accepted_artifact_attachment_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.accepted_artifact_attachment_id
            ));
        }
        if !expected.contains(row.accepted_artifact_attachment_id.as_str()) {
            failures.push(format!(
                "{} is not a held source-needed accepted artifact attachment row",
                row.accepted_artifact_attachment_id
            ));
        }
        ...
        {
            failures.push(format!(
                "{} {} has unsupported accepted attachment review decision",
                row.state, row.route
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing accepted attachment review row for {expected_id}"
            ));
        }
    }
    failures
