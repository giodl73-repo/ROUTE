...
| T2 qualification-action contract | Ready | `data/beck-t2-qualification-actions.csv`; `route beck-t2-qualification-actions --gate` defines keep, keep-primary-review, merge-review, and demote-review semantics |
| T2 game overlay contract | Ready | `data/game/t2-service-overlays.csv` and `data/game/t2-bundle-overlays.csv`; `route game t2-overlays --gate` links T2 service classes to map ids, incident/upgrade/restitch levers, and pavement debt carried from service selection |
...
Canonical campaign rows live in `data/game/campaign-spine.csv`. The prose table is a readable mirror; the gate owns map-id and publication-gate completeness. T2 service overlays use `data/beck-t2-service-standards.csv`, `data/beck-t2-qualification-actions.csv`, `data/game/t2-service-overlays.csv`, `data/game/t2-bundle-overlays.csv`, and `data/game/t2-scenario-hooks.csv` so incidents, upgrades, restitching, duplicate-service choices, and pavement payment pressure can target classes and bundle ids instead of hand-picked line names.
