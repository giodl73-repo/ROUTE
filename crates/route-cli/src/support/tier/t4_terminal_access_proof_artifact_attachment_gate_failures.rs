//! Helper `t4_terminal_access_proof_artifact_attachment_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn t4_terminal_access_proof_artifact_attachment_gate_failures(
    ...
    if expected.is_empty() {
        failures.push(
            "terminal access artifact attachment has no source-needed capture rows".to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "terminal access artifact attachment has {} rows but expected {} capture rows",
            rows.len(),
            expected.len()
        ));
    }
