//! Helper `tier_pavement_funding_evidence_accepted_metadata_intake_gate_failures`.
#[allow(unused_imports)]
use crate::*;

pub(crate) fn tier_pavement_funding_evidence_accepted_metadata_intake_gate_failures(
    ...
    let expected = access_rows
        ...
        .map(|row| row.accepted_metadata_source_access_id.as_str())
        .collect::<std::collections::BTreeSet<_>>();
    if expected.is_empty() {
        failures.push(
            "funding evidence accepted metadata intake has no held source-access rows".to_string(),
        );
    }
    if rows.len() != expected.len() {
        failures.push(format!(
            "funding evidence accepted metadata intake has {} rows but expected {} source-access rows",
            rows.len(),
            expected.len()
        ));
    }
    ...
    for row in rows {
        if row.accepted_metadata_intake_id.trim().is_empty()
            || row.accepted_metadata_source_access_id.trim().is_empty()
        ...
        {
            failures.push(format!(
                "{} {} has incomplete accepted metadata intake row",
                row.state, row.route
            ));
        }
        if !seen.insert(row.accepted_metadata_source_access_id.clone()) {
            failures.push(format!(
                "{} appears more than once",
                row.accepted_metadata_source_access_id
            ));
        }
        if !expected.contains(row.accepted_metadata_source_access_id.as_str()) {
            failures.push(format!(
                "{} is not a held accepted metadata source-access row",
                row.accepted_metadata_source_access_id
            ));
        }
        ...
        {
            failures.push(format!(
                "{} {} has unsupported accepted metadata intake status",
                row.state, row.route
            ));
        }
    ...
    for expected_id in expected {
        if !seen.contains(expected_id) {
            failures.push(format!(
                "missing accepted metadata intake row for {expected_id}"
            ));
        }
    }
    failures
