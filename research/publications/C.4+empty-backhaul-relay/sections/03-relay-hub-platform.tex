...
The relay hub matching problem is a bipartite assignment problem. At peak hour at Chicago (I-80/I-90), approximately 2,700 available loads compete for approximately 1,800 arriving relay drivers. The Hungarian algorithm \citep{Kuhn1955} solves this optimally in $O(n^3)$ time, which at $n = 2{,}700$ requires approximately 19.7 billion operations per hub per hour --- computationally prohibitive for real-time dispatch.

Instead, we propose a \textbf{greedy auction mechanism} \citep{Bertsekas1988auction} that achieves near-optimal matching in $O(n \log n)$ time. The auction proceeds in rounds: each unmatched load bids on available drivers by submitting a price equal to its value minus its current assignment price; drivers are assigned to the highest-bidding load. The mechanism converges in at most $O(n)$ rounds with price increments $\epsilon > 0$, achieving a matching within $n \epsilon$ of optimal. At hub scale ($n = 2{,}700$, $\epsilon = \$0.01$), the total optimality gap is \$27 --- negligible relative to the \$100--150 per-swap fee. Empirically, auction mechanisms converge in 8--15 rounds in simulation, yielding sub-millisecond dispatch latency on commodity hardware.

The greedy auction also handles the relay marketplace's real-time arrival pattern more naturally than batch Hungarian matching: new drivers arriving mid-auction enter as bidders at the current price level, and existing assignments are revised only if a new bid dominates. This produces an approximately stable matching under continuous arrival \citep{Roth2002} without requiring complete restarts.
