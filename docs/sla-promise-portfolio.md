...
`data/t1-sla-candidate-universe.csv` is the candidate pool, and
`data/t1-sla-candidate-pairs.csv` is the ranked cut-line artifact generated by
`route t1-sla-candidate-pairs --gate`.

`data/t1-sla-pairs.csv` is the selected top-25 slice consumed by the T1 line
selector. It should remain a top-25 portfolio until a deliberate budget change
is made.
...
benefit decision, not a hidden manual omission. In the current ranked artifact,
the first dropped pair is `CHI-MIA-36`, which is covered by the selected
reverse-direction `MIA-CHI-36` promise.
...
`required_routes` and `required_stops` are audit claims. If they change, the
candidate-pair ranking, T1 line selector, stop selector, Beck alignment, and
feedback docket must be regenerated.
