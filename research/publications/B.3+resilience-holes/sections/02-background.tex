...
D1 is a composite climate vulnerability score incorporating three exposure components: (1) FEMA National Flood Hazard Layer (NFHL) Special Flood Hazard Area miles as a share of total corridor miles, reflecting baseline flood exposure probability; (2) mean annual closure events from FHWA incident records and Caltrans/WSDOT/LADOTD operational logs \citep{Caltrans2023, FHWA_freight2023}; and (3) a climate trajectory adjustment reflecting whether the 2050 exposure under NOAA intermediate sea-level-rise and precipitation-intensity scenarios is substantially higher than the 2024 baseline \citep{FEMA_NFHL} \citep{ROUTE_D1}.

The D1 scores used in this analysis are drawn from Paper D.1 \citep{ROUTE_D1}, which maps climate exposure for all 227 corridors using FEMA NFHL flood zone overlay and historical closure frequency data. B.3's compound exposure thresholds require D.1 to be finalized before the compound corpus can be validated.
