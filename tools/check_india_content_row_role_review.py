#!/usr/bin/env python3
"""Gate India role review for content-row validation outputs."""
...
LEDGER = ROOT / "data" / "international-india-content-row-role-review-001.csv"
...
def main() -> int:
    ...
    failures: list[str] = []
    if fields != FIELDS:
        failures.append("India content-row role review columns do not match contract")
    if len(rows) != 5:
        failures.append("India content-row role review must have five role rows")
    if {row["role_lane"] for row in rows} != REQUIRED_ROLES:
        failures.append("India content-row role review missing required role lanes")
    for row in rows:
    ...
    if failures:
        print("India content-row role review gate: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("India content-row role review gate: PASS")
