...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | Three simulated G0 playtests and `docs/game/des-moines-diamond-playtest-synthesis.md` show the topology aha landing under capacity-first, connector-first, and evidence-first routes; next step is G0 panel pass plus `route game scenarios` / `inspect` implementation |
