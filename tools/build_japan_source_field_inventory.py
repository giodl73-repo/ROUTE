#!/usr/bin/env python3
"""Build Japan source field inventory."""
...
PROBE = ROOT / "data" / "international-japan-source-payload-probe-001.csv"
OUTPUT = ROOT / "data" / "international-japan-source-field-inventory-001.csv"
...
SOURCE_FAMILIES = {
    "JPN-SRC-001": "road_bureau_context",
    "JPN-SRC-002": "road_statistics_context",
    "JPN-SRC-003": "road_traffic_census_context",
    "JPN-SRC-004": "geospatial_road_context",
    "JPN-SRC-005": "port_system_context",
    "JPN-SRC-006": "port_classification_context",
    "JPN-SRC-007": "hierarchy_fixture_context",
    "JPN-SRC-SLA-001": "service_targets",
}

FIELD_CANDIDATES = {
    "JPN-SRC-001": "road administration context; road bureau title; source owner; access note",
    "JPN-SRC-002": "road category; road length; statistic period; table title; access note",
    "JPN-SRC-003": "statistics code; census title; survey table; ministry in charge; access note",
    "JPN-SRC-004": "transportation layer; road feature class; dataset version; download format; access note",
    "JPN-SRC-005": "ports bureau context; port policy title; source owner; access note",
    "JPN-SRC-006": "port classification; port type; legal context; access note",
    "JPN-SRC-007": "candidate tier; service role; readiness basis; evidence label; claim boundary",
    "JPN-SRC-SLA-001": "target id; target class; assumption label; local basis; numeracy review",
}
...
def main() -> None:
    ...
    for index, source_id in enumerate(SOURCE_FAMILIES, start=1):
        ...
        if source_id == "JPN-SRC-SLA-001":
            status = "held_no_payload"
            label = "held"
            basis = "not-fetchable target assumption row"
            next_action = "create Japan target posture before any service target promotion"
        elif source_id == "JPN-SRC-007":
        ...
        rows.append(
            {
                "inventory_id": f"JPN-FIELD-{index:03d}",
                "source_id": source_id,
