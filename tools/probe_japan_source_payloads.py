#!/usr/bin/env python3
"""Probe Japan source-payload URLs without accepting evidence."""
...
OUTPUT = ROOT / "data" / "international-japan-source-payload-probe-001.csv"
USER_AGENT = "ROUTE-Japan-payload-probe/0.1 evidence-not-accepted"
TIMEOUT_SECONDS = 20
SAMPLE_BYTES = 65536
...
def main() -> int:
    ...
    for access in read_csv(ACCESS):
        ...
        rows.append(
            {
                "probe_id": f"JPN-PROBE-{source_id.replace('JPN-SRC-', '').replace('-', '')}",
                "source_id": source_id,
