...
| Interstate Tycoon turns pressure-test proof into a playable standard-by-standard demo | 🔄 partial | `docs/game/route-game-cli-design.md` now defines the G1 command/state/scoring contract; next step is a blind G0 playtest plus `route game scenarios` / `inspect` implementation |
