#!/usr/bin/env python3
"""Render India logistics-spine candidate hierarchy v2."""
...
INPUT = ROOT / "data" / "international-india-candidate-hierarchy-v2.csv"
OUT = ROOT / "maps" / "international" / "india-candidate-hierarchy-v2.svg"
W = 1700
H = 1040
MARGIN = 105
TIER_STYLE = {
    "T1": {"color": "#2563eb", "width": 10, "label": "T1 candidate national logistics spine"},
...
def main() -> None:
    ...
    svg = [
        ...
        text(70, 82, "India candidate logistics hierarchy v2", 32, "#f8fafc"),
        text(70, 116, "Iteration result: national logistics spine, regional connectors, port feeders; official, SLA, construction, ROI, approval, and validation claims held.", 15, "#cbd5e1"),
    ]
    for row in rows:
    ...
    svg.append(f'<rect x="{panel_x}" y="{panel_y}" width="400" height="610" rx="12" fill="#020617" stroke="#334155"/>')
    svg.append(text(panel_x + 24, panel_y + 38, "Iteration changes", 20, "#f8fafc"))
    for idx, change in enumerate(["Ahmedabad-Mumbai repaired to T1", "Mumbai-Pune up-tiered to T1", "Lucknow and Bengaluru added", "Port access separated as T3", "Hyderabad access candidate added"]):
    ...
    svg.append(text(panel_x + 24, panel_y + 480, "Still held", 18, "#f8fafc"))
    for idx, item in enumerate(["NHAI/NH source-bound parsing", "port terminal source packs", "monsoon/resilience constraints", "target/SLA proof"]):
        svg.append(text(panel_x + 34, panel_y + 516 + idx * 26, f"- {item}", 13, "#cbd5e1"))
    svg.append(text(70, H - 54, "Held claims: official Indian corridor, national/state approval, guaranteed SLA, construction, ROI, eligibility, compliance, endorsement, external validation.", 14, "#fbbf24"))
    svg.append("</svg>")
